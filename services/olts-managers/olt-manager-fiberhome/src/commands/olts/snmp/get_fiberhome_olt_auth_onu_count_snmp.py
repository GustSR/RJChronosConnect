...
class GetFiberhomeOltAuthOnuCountSnmpCommand(OLTCommand):
    """Get authorized ONU count via SNMP (Fiberhome AN5116)."""

    OID_AUTH_ONU_COUNT = "1.3.6.1.4.1.5875.800.3.9.4.6"
    ...
    def execute(self, connection_manager=None, olt_version: str = None) -> Dict[str, Any]:
        async def _execute_async() -> Dict[str, Any]:
            ...
            error_indication, error_status, error_index, var_binds = await v3arch.get_cmd(
                ...
                transport,
                context,
                v3arch.ObjectType(v3arch.ObjectIdentity(self.OID_AUTH_ONU_COUNT)),
            )
            ...
            return {"authorized_onu_count": count, "raw_value": str(raw_value)}
