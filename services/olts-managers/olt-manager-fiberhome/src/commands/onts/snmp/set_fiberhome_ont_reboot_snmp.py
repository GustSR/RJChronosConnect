...
class SetFiberhomeOntRebootSnmpCommand(OLTCommand):
    """Reboot an ONU via SNMP set (Fiberhome AN5116)."""

    OID_ONU_REBOOT = "1.3.6.1.4.1.5875.800.3.10.1.1.61"
    ...
    def execute(self, connection_manager=None, olt_version: str = None) -> Dict[str, Any]:
        async def _execute_async() -> Dict[str, Any]:
            oid = f"{self.OID_ONU_REBOOT}.{self.onu_index}"
