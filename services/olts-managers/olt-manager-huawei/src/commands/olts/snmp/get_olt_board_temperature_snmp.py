...
class GetOltBoardTemperatureSnmpCommand(OLTCommand):
    """Get board temperature via SNMP (Huawei MA5800)."""

    OID_BOARD_TEMPERATURE = "1.3.6.1.4.1.2011.2.6.7.1.1.2.1.10"
    ...
    def execute(self, connection_manager=None, olt_version: str = None) -> Dict[str, Any]:
        async def _execute_async() -> Dict[str, Any]:
            oid = f"{self.OID_BOARD_TEMPERATURE}.{self.board_index}"
