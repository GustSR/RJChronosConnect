...
const Clientes: React.FC = () => {
  ...
  const convertProvisionedToCliente = (onu: any): Cliente => ({
    id: onu.id,
    status: onu.status,
    nome: onu.clientName,
  ...
  useEffect(() => {
    let filtered = clientes.filter(cliente => {
      ...
      const matchesBoard = boardFilter === 'all' || cliente.board === boardFilter;
      const matchesPort = portFilter === 'all' || cliente.port === portFilter;
      const matchesVlan = vlanFilter === 'all' || cliente.vlan === vlanFilter;
      const matchesTipo = tipoOnuFilter === 'all' || cliente.tipoOnu === tipoOnuFilter;
      
      let matchesSinal = true;
      if (sinalFilter === 'excelente') matchesSinal = cliente.sinal >= -15;
      else if (sinalFilter === 'bom') matchesSinal = cliente.sinal >= -20 && cliente.sinal < -15;
      else if (sinalFilter === 'regular') matchesSinal = cliente.sinal >= -25 && cliente.sinal < -20;
      else if (sinalFilter === 'ruim') matchesSinal = cliente.sinal < -25;

      return matchesSearch && matchesStatus && matchesOlt && matchesBoard && 
             matchesPort && matchesVlan && matchesTipo && matchesSinal;
    });

    setFilteredClientes(filtered);
    setPage(0);
  }, [searchTerm, statusFilter, oltFilter, boardFilter, portFilter, vlanFilter, tipoOnuFilter, sinalFilter, clientes]);
  ...
  const getStatusChip = (status: string) => {
    const statusConfig = {
      online: { label: 'Online', color: 'success' as const, icon: <Circle fontSize="small" /> },
      offline: { label: 'Offline', color: 'error' as const, icon: <Circle fontSize="small" /> },
      powered_off: { label: 'Powered Off', color: 'warning' as const, icon: <Circle fontSize="small" /> },
      admin_disabled: { label: 'Admin Disabled', color: 'default' as const, icon: <Circle fontSize="small" /> },
    };
          ...
          <Grid item xs={12} sm={6} md={3}>
            <AnimatedCard delay={100}>
              <Card sx={{ 
                ...
                <CardContent sx={{ p: 3 }}>
                  <Typography variant="body2" color="text.secondary" fontWeight={500} sx={{ mb: 1, fontSize: '14px' }}>
                    Total de Clientes
                  </Typography>
                  <Typography variant="h4" fontWeight="700" color="text.primary" sx={{
                    background: 'linear-gradient(135deg, #6366f1, rgba(99, 102, 241, 0.7))',
                    backgroundClip: 'text',
            ...
            <AnimatedCard delay={200}>
              <Card sx={{ 
                ...
                <CardContent sx={{ p: 3 }}>
                  ...
                  <Typography variant="h4" fontWeight="700" color="text.primary" sx={{
                    background: 'linear-gradient(135deg, #ef4444, rgba(239, 68, 68, 0.7))',
                    backgroundClip: 'text',
                    WebkitBackgroundClip: 'text',
                    WebkitTextFillColor: 'transparent',
                  }}>
            ...
            <AnimatedCard delay={300}>
              <Card sx={{ 
                ...
                <CardContent sx={{ p: 3 }}>
                  ...
                  <Typography variant="h4" fontWeight="700" color="text.primary" sx={{
                    background: 'linear-gradient(135deg, #10b981, rgba(16, 185, 129, 0.7))',
                    backgroundClip: 'text',
                    WebkitBackgroundClip: 'text',
                    WebkitTextFillColor: 'transparent',
                  }}>
              ...
              <Card sx={{ 
                ...
                <CardContent sx={{ p: 3 }}>
                  ...
                  <Typography variant="h4" fontWeight="700" color="text.primary" sx={{
                    ...
                    {clientes.filter(c => c.status === 'powered_off' || c.status === 'admin_disabled' || c.sinal < -25).length}
                  </Typography>
                </CardContent>
              </Card>
            </AnimatedCard>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={3}>
                <TextField
                  fullWidth
                  label="Pesquisar"
                  placeholder="Nome, SN, IP, Endereço..."
                  value={searchTerm}
              ...
              <Grid item xs={6} sm={4} md={1.5}>
                <FormControl fullWidth>
                  ...
                  >
                    <MenuItem value="all">Todas</MenuItem>
                    <MenuItem value="105">105 - PPPoE</MenuItem>
                    <MenuItem value="106">106 - DHCP</MenuItem>
                    <MenuItem value="107">107 - Static</MenuItem>
                    <MenuItem value="108">108 - Bridge</MenuItem>
                  </Select>
                </FormControl>
              </Grid>
                    ...
                    filteredClientes
                      ...
                      .map((cliente) => (
                        <TableRow key={cliente.id} hover>
                          ...
                          <TableCell>
                            <Chip
                              label={cliente.modo === 'routing' ? 'Routing' : 'Bridge'}
                              size="small"
                              color={cliente.modo === 'routing' ? 'primary' : 'secondary'}
                              variant="outlined"
                            />
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            <Typography variant="caption">
                              {new Date(cliente.dataAutenticacao).toLocaleDateString('pt-BR')}
                            </Typography>
                          </TableCell>
                          <TableCell>
